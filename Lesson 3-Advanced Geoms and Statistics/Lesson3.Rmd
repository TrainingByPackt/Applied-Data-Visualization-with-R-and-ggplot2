...
One can study trends in data by looking at scatter plots between two variables.
This reveals if one variable is related to another variable. If variable A
increases as variable B increases, its a positive correlation. If variable A
decreases as variable B increases, its a negative correlation. If there is
no clear trend, then its a zero correlation. The values of the correlation
coefficient therefore can lie between -1 to +1.

To study if there is a trend, once can add a trend line by fitting the points,
and studying the "goodness of fit". 

For example, we can add a linear regression line to a scatter plot. 

To do this, we add stat_smooth() and tell it to use method=lm. This instructs it to fit the data with the lm() (linear model) function. 

See the two commands below. The points are color differentitated with country. Note the difference in the two plots. In the first case, the color aesthetic is
provided to geom_point() while in the second case, its provided when we define the aesthetic for the plot. In the second case, fitted lines are drawn separately for each country.

The points show a positive trend as is also confirmed by the fitted line. In other words, Electricity consumption is positively correlated to the GDP of a country.
...
Instead of making individual scatter plots, we could also study the correlation
of all variables at once using a correlation plot.
